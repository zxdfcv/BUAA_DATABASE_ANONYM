<template>
  ...
  const getThingDetail = () => {
    ...
    console.log(video)
    canteenDetailApi({id: thingId.value}).then(res => {
  ...
  const collect = () => {
    ...
    if (username) {
      ...
      addCollectCanteen({canteenId: thingId.value, username: username}).then(res => {
        message.success(res.msg)
        getThingDetail()
      }).catch(err => {
        console.log('收藏餐厅失败')
...
const share = () => {
  ...
  const sendComment = () => {
    ...
    if (userId) {
      createCommentApi({content: text, canteen: thingId.value, user: userId}, {}).then(res => {
        getCommentList()
      }).catch(err => {
        console.log(err)
      })
  ...
  const sendReply = (parentId) => {
    ...
    let userId = userStore.user_id
    if (userId) {
      createCommentApi({content: text, canteen: thingId.value, user: userId}, {parentId: parentId}).then(res => {
        getCommentList()
      }).catch(err => {
          console.log(err)
      })
  ...
  const getCommentList = () => {
    listThingCommentsApi({ thingId: thingId.value, order: order.value, type: "canteen"})
    .then((res) => {
