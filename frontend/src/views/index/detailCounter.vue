<template>
          ...
          <div style="position: relative;">
            <div class="thing-infos-view">
              <div class="thing-infos">
                ...
                <img :src="BASE_URL + detailData.cover" width="800" height="500">
              ...
              <div class="comments-list">
                <div class="comment-item" v-for="item in commentData">
                  <div class="flex-item flex-view">
                    <img v-if="SelfAvatar(item.user_avatar.avatar)" :src="BASE_URL + item.user_avatar.avatar" class="avator" @click="toggleIsShow(item.id, item.id)">
                  ...
                  <div v-if="item.reply_count > 0">
                    ...
                    <div class="comment-item-sub" v-for="reply in item.replies.data">
                      <div class="flex-item flex-view">
                        <img v-if=SelfAvatar(reply.user_avatar.avatar) :src="BASE_URL + reply.user_avatar.avatar" class="avator" @click="toggleIsShow(item.id, reply.id)">
  ...
  const getThingDetail = () => {
    ...
    console.log(video)
    counterDetailApi({id: thingId.value}).then(res => {
  ...
  const collect = () => {
    ...
    if (username) {
      ...
      addCollectCounter({classificationId: thingId.value, username: username}).then(res => {
        message.success(res.msg)
        getThingDetail()
      }).catch(err => {
        console.log('收藏柜台失败')
...
const share = () => {
  ...
  const sendComment = () => {
    ...
    if (userId) {
      createCommentApi({content: text, classification: thingId.value, user: userId}, {}).then(res => {
        getCommentList()
      }).catch(err => {
        console.log(err)
      })
  ...
  const sendReply = (parentId) => {
    ...
    let userId = userStore.user_id
    if (userId) {
      createCommentApi({content: text, classification: thingId.value, user: userId}, {parentId: parentId}).then(res => {
        getCommentList()
      }).catch(err => {
          console.log(err)
      })
  ...
  const getCommentList = () => {
    listThingCommentsApi({ thingId: thingId.value, order: order.value, type: "classification"})
    .then((res) => {
